...
8. O texto recebido aparece no editor de 120–190 px e pode ser corrigido, apagado, revisado com IA (concordância, regência, crase e pontuação via Gemini assíncrono), verificado por sublinhado ondulado com balão flutuante de sugestões e menu de contexto e copiado. `Diagnóstico` permanece visível à direita desde a primeira pintura, com abas `Áudio`, `Payload`, `Retorno` e `Consumo` e gráfico abaixo; não existe botão de toggle/dock. O cabeçalho mostra contagem de atalhos, engrenagem e tela cheia. Em X11, `Enviar ao terminal` cola via `Ctrl+Shift+V` sem Enter; em Wayland ou sem `xdotool`, `Copiar texto` é o fallback.

Nenhuma chave é mostrada em label, tooltip, status, exceção, log, arquivo, banco ou argumento. O SQLite armazena apenas `last_microphone_identity`, `recording_mouse_button`, `recording_keyboard_shortcut`, `gemini_model`, `spellcheck_enabled`, `spellcheck_ignored_words` e metadados allowlisted de tokens, preservando nulos; nunca armazena chave, áudio, base64, prompt, transcrição, resposta textual, exceção bruta ou preço.
...
| `src/falafacil/config.py` | Define `Settings`, `DEFAULT_MODEL` (`gemini-3.5-flash-lite`), `MODEL_CHOICES`, precedência entre ambiente, fallback persistido e padrão, e mensagem de configuração. |
...
| `src/falafacil/storage.py` | SQLite schema v1 com preferências allowlisted de microfone, modelo Gemini (`gemini_model`), mouse, teclado, corretor ortográfico (`spellcheck_enabled`, `spellcheck_ignored_words`) e histórico allowlisted de tokens. |
...
| `src/falafacil/spellcheck.py` | Define `LocalSpellChecker`, wrapper `ctypes` fail-soft para `libenchant-2.so.2` com dicionário `pt_BR`, tokenização sem URLs/caminhos/e-mails, função utilitária `utf16_code_unit_offsets` para alinhamento UTF-16 com caracteres não-BMP, sugestões e gestão de palavras ignoradas. |
| `src/falafacil/spell_highlighter.py` | Define `SpellHighlighter(QSyntaxHighlighter)` com sublinhado ondulado vermelho (`SpellCheckUnderline`) em palavras desconhecidas no editor. |
...
| `src/falafacil/transcription.py` | Define `GeminiTranscriber`, `TranscriptionDebug` (com `TokenUsage`), `TokenUsage`, `TranscriptionWorker`, `ProofreadingWorker`, `TranscriptionError`, os prompts de fala e revisão profunda `PROOFREADING_PROMPT`, `INLINE_LIMIT_BYTES` e `REQUEST_TIMEOUT_MS`; aceita chave injetada na criação do cliente e extrai metadados de tokens de `interaction.usage`. |
| `src/falafacil/ui.py` | Janela em splitter com diagnóstico permanente, engrenagem/configurações em seis grupos, tela cheia, botão de revisão com IA, balão flutuante de sugestões e menu de contexto ortográficos, dois atalhos ACK-gated, áudio em memória, transcrição, clipboard e fechamento ordenado. |
...
- O SQLite armazena somente `last_microphone_identity`, `recording_mouse_button`, `recording_keyboard_shortcut`, `gemini_model`, `spellcheck_enabled`, `spellcheck_ignored_words` e metadados allowlisted de consumo; nunca armazena chave, áudio, base64, prompt, transcrição, resposta, exceção bruta ou preço.
