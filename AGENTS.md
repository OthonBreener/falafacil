...
Aplicativo desktop local para Ubuntu que grava fala em português do Brasil, envia o áudio ao Gemini, exibe uma transcrição editável, copia o texto para o clipboard e, em uma sessão X11 compatível, pode colá-lo no terminal ativo ou de origem salvo. O fluxo não executa comandos e não envia Enter.
...
1. `falafacil.app:main` cria uma única `QApplication`, define o nome da organização, o nome da aplicação e a versão da aplicação (`app.setApplicationVersion(__version__)`, usando `falafacil.__version__` como fonte única da versão `0.3.0`) antes de usar configurações, inicializa de forma fail-soft o `LocalStore` no diretório de dados (`AppDataLocation`), tenta ler o modelo Gemini persistido (`get_gemini_model()`), tenta ler a chave persistida no Secret Service pelo `KeyringApiKeyStore`, detecta de forma fail-soft a instalação Homebrew (`detect_homebrew_installation()`), constrói de forma fail-soft o controlador `HomebrewUpdateController` quando a instalação for detectada, injeta-o na `MainWindow` e registra o desktop entry do usuário (`install_user_desktop_entry()`) a partir da mesma instalação antes de exibir `MainWindow` (execução por código-fonte ou modo developer não realiza escrita automática). Tenta construir o transcritor inicial com `(api_key, model)`: se a inicialização do Gemini falhar, a UI abre sem transcritor e exibe uma mensagem recuperável sanitizada para revisar chave ou modelo nas Configurações. Falhas do chaveiro, de detecção, de criação do controlador, do banco ou do desktop entry não impedem a abertura da janela.
2. `Settings.from_env(*, fallback_api_key=..., fallback_model=...)` escolhe o modelo pela precedência `GEMINI_MODEL` (valor não vazio) > modelo persistido > `DEFAULT_MODEL` (`gemini-3.5-flash-lite`), registrando `model_from_environment`, e escolhe a chave pela precedência `GEMINI_API_KEY`, depois `GOOGLE_API_KEY`, depois a chave persistida. A chave permanece fora de `repr` e comparações de `Settings`.
...
4. Ao detectar microfones, a UI utiliza metadados locais do PortAudio e a função de prioridade `choose_input_device()`: headset sempre tem precedência; se não houver headset, tenta o dispositivo atual ou a memória do último microfone usado salva no `LocalStore`; em seguida, tenta dispositivo interno, padrão do sistema ou o primeiro disponível. Se nenhum dispositivo existir, a gravação é desabilitada com aviso recuperável. A gravação requer chave API configurada, transcritor funcional e microfone disponível. Ao clicar em `Gravar` ou acionar a gravação pelo atalho de mouse/teclado, `AudioRecorder` abre um `sounddevice.InputStream` no microfone selecionado, mono e `int16`. Após o início bem-sucedido da captura, a identidade normalizada do dispositivo é persistida no `LocalStore`. Tenta `16 kHz`; se o dispositivo só aceitar outra taxa nativa, usa reamostragem fora do callback.
5. A engrenagem abre `Configurações` com seis grupos: `Chave API`, `Modelo Gemini`, `Atalho do mouse`, `Atalho do teclado`, `Corretor ortográfico` e `Atualizações`. O seletor de modelo exibe a allowlist ordenada (`gemini-3.5-flash-lite`, `gemini-3.7-flash`, `gemini-3.8-flash`); aplicar com chave ativa constrói o novo transcritor via factory `(api_key, model)` antes da troca de estado (servindo como recuperação caso a construção inicial tenha falhado); sem chave, troca apenas `Settings`; falha da factory preserva tudo, e falha do SQLite mantém a escolha na sessão com mensagem sanitizada. O seletor e o botão são desabilitados durante `RECORDING`/`TRANSCRIBING` ou quando `GEMINI_MODEL` estiver definido com valor não vazio no ambiente. Mouse aceita somente botões laterais/central seguros — um botão rejeitado explica o motivo no diálogo em vez de ficar em espera, e uma captura sem entrada reconhecida orienta remapeamento no fabricante. O grupo `Corretor ortográfico` exibe o status de disponibilidade do dicionário local (`pt_BR` via `libenchant-2.so.2`) e permite alternar o sublinhado visual de palavras desconhecidas no editor. O grupo `Atualizações` exibe a versão instalada (`Versão instalada: <versão>`), o status de atualização, barra de progresso indeterminada durante a execução e o botão literal `Instalar atualizações`. Em instalações Homebrew válidas, o botão aciona o ciclo assíncrono do controlador (`brew update-if-needed` -> `brew outdated` -> `brew upgrade` -> probe); se já atualizado, informa `Você já usa a versão mais recente.`; ao concluir a instalação com sucesso, abre um diálogo com as opções `Reiniciar agora` (aciona `restart()` e fecha a aplicação preservando o encerramento ordenado) e `Mais tarde` (mantém a aplicação aberta sem reiniciar). Em execuções a partir do código-fonte ou instalações de desenvolvimento sem marker válido, o botão fica desabilitado com o status `Instale o FalaFácil com: brew install OthonBreener/falafacil/falafacil`. A atualização do pacote do produto permanece independente da integração privilegiada de atalhos globais.
6. Ao parar, o stream é encerrado, os bytes PCM são serializados como WAV em memória e RMS/pico e integridade de status são validados. Captura vazia, abaixo de `MIN_RMS_LEVEL` ou com integridade de status comprometida fica no diagnóstico e entra em erro sem chamada de rede. Uma captura utilizável sobrevive a erros de limpeza ao parar/fechar ou falhas transitórias do Gemini para permitir nova tentativa idêntica. A ação principal unificada segue o fluxo sensível ao estado: `Gravar` → `Parar e revisar áudio` → `Enviar para Gemini`. Um botão dedicado `Descartar e gravar novamente` permite reiniciar a gravação preservando a captura anterior caso o novo início, parada, nível, status ou limpeza falhe. `Reproduzir áudio` alterna com `Parar reprodução` via `QBuffer`/`QMediaPlayer`, sendo interrompido e desanexado com segurança antes de ações incompatíveis ou encerramento (bloqueando a ação se a liberação falhar). `Enviar para Gemini` é a única ação que inicia a transcrição.
7. O envio ao Gemini ocorre em um `QThread` por meio de `TranscriptionWorker`, usando `client.interactions.create(model=..., input=[prompt, audio], store=False)` em ordem prompt→áudio direta, sem camadas explícitas de cache. Se o bloco `Transcrição atual` contiver texto não vazio, o envio é impedido, o áudio pendente é preservado e a UI exibe `Copie e arquive a transcrição atual antes de enviar outro áudio.` A resposta da Interactions API fornece `interaction.usage`, que é encapsulado em `TokenUsage` dentro de `TranscriptionDebug` com `total_cached_tokens` observado do cache implícito da API. Os sinais de sucesso ou falha retornam ao thread da interface com o `TranscriptionDebug`, sem acessar widgets ou banco de dados no worker. No thread principal, a UI registra o consumo no `LocalStore` (com outcome `success` ou `error`), atualiza os dados textuais de consumo da chamada e acumulado e atualiza o gráfico de consumo de tokens por chamada (`TokenUsageChart`). Na transcrição bem-sucedida, o texto é copiado automaticamente para a área de transferência, movido para o editor de última mensagem (substituindo a mensagem anterior única), o bloco de transcrição atual é limpo e o áudio WAV pendente é liberado.
8. A área de trabalho divide-se em dois blocos: **Transcrição atual** (com ação `Copiar e arquivar`) e **Última mensagem — somente nesta sessão** (editor editável em memória, com ações `Revisar com IA`, `Copiar novamente`, `Enviar ao terminal` e `Apagar`). O corretor ortográfico local (`libenchant-2.so.2` / `hunspell-pt-br`), o sublinhado ondulado vermelho, o balão flutuante de sugestões e o menu de contexto aplicam-se exclusivamente ao editor de última mensagem. O botão `Revisar com IA` envia o texto exato sem cortes ao Gemini com `store=False` (preservando espaços e quebras de linha no payload e na contagem de bytes), substitui e copia o resultado e suporta Desfazer (Undo). `Diagnóstico` permanece visível à direita desde a primeira pintura, com abas `Áudio`, `Payload`, `Retorno` e `Consumo` e gráfico abaixo; não existe botão de dock/toggle. O cabeçalho mostra contagem de atalhos, engrenagem e tela cheia. Em X11, `Enviar ao terminal` revalida o alvo salvo (ou detecta a janela ativa), ativa a janela de forma síncrona com `xdotool windowactivate --sync` (apenas para alvo salvo), confirma o foco e cola via `Ctrl+Shift+V` sem Enter; se o alvo salvo for inválido, a operação falha de forma fechada sem colar ou redetectar; em Wayland ou sem `xdotool`, `Copiar novamente` / `Copiar e arquivar` é o fallback. Nenhum texto de qualquer dos editores é persistido em disco.

Nenhuma chave é mostrada em label, tooltip, status, exceção, log, arquivo, banco ou argumento. O SQLite armazena apenas `last_microphone_identity`, `recording_mouse_button`, `recording_keyboard_shortcut`, `gemini_model` (aceitando `gemini-3.5-flash-lite`, `gemini-3.7-flash` ou `gemini-3.8-flash`), `spellcheck_enabled`, `spellcheck_ignored_words` e metadados allowlisted de tokens, preservando nulos; nunca armazena chave, áudio, base64, mensagens da sessão, alvos de terminal de origem salvos, prompt, transcrição, resposta textual, exceção bruta ou preço.
...
|---|---|
| `IDLE` | Janela pronta para iniciar uma gravação; sem chave, microfone ou transcritor funcional, exibe a mensagem correspondente. |
| `RECORDING` | `AudioRecorder` mantém um stream aberto e a ação principal exibe `Parar e revisar áudio`. |
| `AUDIO_READY` | Há um `AudioCapture` válido em memória, ainda não enviado; reprodução, envio explícito via `Enviar para Gemini` e descarte com nova gravação estão disponíveis. |
| `TRANSCRIBING` | O WAV já foi produzido e `TranscriptionWorker` executa a transcrição no `QThread`; a ação principal exibe `Transcrevendo…` e ações dependentes ficam desabilitadas. |
| `READY` | Há uma mensagem arquivada no editor de última mensagem da sessão atual, pronta para revisão com IA, nova cópia ou envio ao terminal. |
| `ERROR` | Uma operação falhou; a mensagem fica no status e a janela pode ser usada novamente, preservando a captura anterior quando utilizável. |

O atalho global para iniciar a gravação não rouba foco (capturando opcionalmente o terminal X11 de origem); ao parar a gravação, o atalho global restaura a janela e a traz à frente para revisão do áudio; uma ativação global subsequente no estado `AUDIO_READY` despacha o envio para o Gemini sem nova elevação de foco. Gatilhos globais de mouse e teclado compartilham debounce de 0,35 s; acionamentos manuais na interface e a tecla `Espaço` (fora de campos de texto) atuam imediatamente sem debounce. A ponte de atalhos implementa reconexão automática após 1000 ms em caso de desconexão inesperada, erro ou falha de protocolo.
...
- `google-genai>=2.3.0` (versão travada `2.22.0` no `poetry.lock`), usando `from google import genai` e a Interactions API.
...
`libportaudio2` é um requisito nativo no ambiente de desenvolvimento/compilação no Ubuntu. No bundle Linux x86_64 distribuído via Homebrew, a biblioteca `libportaudio.so.2` é embutida no executável pelo PyInstaller, não exigindo `apt install libportaudio2` na máquina do usuário final. `libenchant-2-2` e `hunspell-pt-br` são requisitos nativos opcionais do sistema para o corretor ortográfico local pt-BR, com fail-soft absoluto (se ausentes, a aplicação funciona normalmente e a UI explica como instalá-los nas Configurações). `xdotool` é um requisito opcional do sistema somente para colagem em terminal X11; não é uma dependência Python do projeto. O ambiente de desktop deve fornecer um backend Secret Service para persistir a chave; sem ele, a configuração continua válida somente na sessão atual.
...
| `.agents/skills/falafacil-release/` | Localização canônica da skill de release do FalaFácil para agentes (com symlinks relativos em `.claude/skills/falafacil-release` e `.omp/skills/falafacil-release`). |
...
| `src/falafacil/app.py` | Define nomes e versão Qt (`__version__`) antes da configuração, inicializa de forma fail-soft o `LocalStore`, carrega modelo/chave persistidos, detecta instalação Homebrew para instanciação do `HomebrewUpdateController` e registro de desktop entry fail-soft antes de `MainWindow.show()`, compõe `Settings`, factory de transcritor `(api_key, model)`, construção fail-soft do transcritor inicial (com mensagem sanitizada se falhar) e `MainWindow`. |
...
| `src/falafacil/config.py` | Define `Settings`, `DEFAULT_MODEL` (`gemini-3.5-flash-lite`), a allowlist ordenada `MODEL_CHOICES` (`gemini-3.5-flash-lite`, `gemini-3.7-flash`, `gemini-3.8-flash`), precedência entre ambiente, fallback persistido e padrão, e mensagem de configuração. |
...
| `src/falafacil/storage.py` | SQLite schema v1 com preferências allowlisted de microfone, modelo Gemini (`gemini_model` com `gemini-3.5-flash-lite`, `gemini-3.7-flash`, `gemini-3.8-flash`), mouse, teclado, corretor ortográfico (`spellcheck_enabled`, `spellcheck_ignored_words`) e histórico allowlisted de tokens; nunca persiste mensagens da sessão, áudio ou alvos de terminal. |
| `src/falafacil/shortcuts.py` | Normalizadores seguros e `InputShortcutBridge`, cliente generation-safe do protocolo ASCII sobre `QLocalSocket` com temporizador de reconexão automática de 1000 ms e cancelamento no fechamento. |
...
| `src/falafacil/spellcheck.py` | Define `LocalSpellChecker`, wrapper `ctypes` fail-soft para `libenchant-2.so.2` com dicionário `pt_BR`, tratamento de resultado 0 como correto, positivo como incorreto e negativo como erro tratado como correto em fail-soft, utilitário `utf16_code_unit_offsets` para alinhamento UTF-16 com caracteres não-BMP, sugestões e gestão de palavras ignoradas. |
| `src/falafacil/spell_highlighter.py` | Define `SpellHighlighter(QSyntaxHighlighter)` com sublinhado ondulado vermelho (`SpellCheckUnderline`) em palavras desconhecidas aplicado exclusivamente ao editor de última mensagem. |
| `src/falafacil/audio.py` | Define `AudioDevice` (com `host_api`, `kind` e `identity` estável), `AudioCapture`, `AudioRecorder`, `MIN_RMS_LEVEL`, `AudioRecorderError`, classificação heurística local, seleção determinística por prioridade, callback de captura, validação de integridade de áudio (rejeitando capturas com status comprometido ou nível insuficiente) e serialização WAV. |
| `src/falafacil/transcription.py` | Define `GeminiTranscriber`, `TranscriptionDebug` (com `TokenUsage`), `TokenUsage`, `TranscriptionWorker`, `ProofreadingWorker`, `TranscriptionError`, os prompts de fala e revisão profunda `PROOFREADING_PROMPT`, envio com `store=False`, limite inline de 20 MiB e timeout de 120 s; aceita chave injetada e extrai metadados de tokens de `interaction.usage`. |
| `src/falafacil/ui.py` | Janela em splitter com diagnóstico permanente, dois blocos de texto (`Transcrição atual` e `Última mensagem — somente nesta sessão`), ação principal unificada (`Gravar` → `Parar e revisar áudio` → `Enviar para Gemini`), botão dedicado de descarte, ações por bloco, revisão com IA assíncrona, balão e menu de contexto ortográficos, captura de terminal X11 de origem no início global, debounce global de 0,35 s, elevação seletiva de foco na parada, reemissão de bindings confirmados com novas gerações e `persist=False` após prontidão do serviço, reconexão de atalhos e fechamento não bloqueante sem `QThread.wait()`. |
| `src/falafacil/terminal.py` | Define `TerminalBridge`, `TerminalTarget`, `TERMINAL_PROCESSES`, captura de alvo de origem X11 na gravação global, revalidação imediata de processo/PID, ativação síncrona de janela (`xdotool windowactivate --sync`), confirmação de ativação e colagem segura via `Ctrl+Shift+V` sem Enter. |
...
- O SQLite armazena somente `last_microphone_identity`, `recording_mouse_button`, `recording_keyboard_shortcut`, `gemini_model` (aceitando `gemini-3.5-flash-lite`, `gemini-3.7-flash` ou `gemini-3.8-flash`), `spellcheck_enabled`, `spellcheck_ignored_words` e metadados allowlisted de consumo; nunca armazena chave, áudio, base64, mensagens da sessão, alvos de terminal salvos, prompt, transcrição, resposta, exceção bruta ou preço.
...
- Mouse e teclado são press-only e generation-safe; gatilhos globais compartilham debounce de 0,35 s; acionamentos manuais e `Space` (fora de campos de texto) não sofrem debounce. O início global não rouba foco (capturando opcionalmente o terminal X11 de origem); a parada global restaura/traz a janela à frente para revisão; nova ativação em `AUDIO_READY` envia ao Gemini sem novo foco. O serviço não usa `grab()`, não suprime eventos, não escreve e não abre rede.
...
- Sem chave configurada ou se a construção inicial do transcritor falhar, a janela abre com a gravação desabilitada e exibe aviso sanitizado; a gravação requer chave API, transcritor funcional e microfone. Configurar chave ou modelo pela UI cria o transcritor via factory `(api_key, model)` antes de substituir `Settings`, servindo como recuperação de falhas iniciais.
...
- O áudio pendente e o `QBuffer` existem somente em memória; um WAV utilizável sobrevive a erros de limpeza ao parar/fechar e a falhas transitórias da API para permitir nova tentativa idêntica; capturas com nível insuficiente ou status comprometido não são enviadas. A substituição via `Descartar e gravar novamente` preserva a captura anterior se o novo início, parada, nível, status ou limpeza falhar. A reprodução alterna e é interrompida e desanexada com segurança antes de ações incompatíveis ou encerramento (bloqueando a ação se a liberação falhar).
- WAVs inline acima de `20 * 1024 * 1024` bytes (`INLINE_LIMIT_BYTES`) são rejeitados antes da chamada ao Gemini. Ambos os envios (`transcribe` e `proofread`) passam `store=False`. Esse fluxo curto não usa Files API.
...
- `TerminalBridge` só atua quando `XDG_SESSION_TYPE=x11`, `xdotool` está disponível, e há um alvo capturado na gravação global ou uma janela ativa com processo validado na allowlist `TERMINAL_PROCESSES`: `gnome-terminal-server`, `konsole`, `kitty`, `alacritty`, `xfce4-terminal`, `lxterminal`, `xterm`, `wezterm-gui` ou `foot`.
- A detecção e a revalidação usam o PID da janela e `/proc/<pid>/comm`; nunca se deve confiar apenas no título da janela.
- O envio ao terminal atua com `Ctrl+Shift+V` sem executar comandos nem enviar Enter: para alvo salvo na gravação global, revalida o alvo, ativa a janela de forma síncrona com `xdotool windowactivate --sync` e confirma a ativação, falhando de forma fechada sem colar ou redetectar se for inválido; para a janela atualmente ativa (sem alvo salvo), detecta e valida a janela ativa colando diretamente sem alteração de foco.
- Em Wayland, a integração automática de terminal é indisponível por contrato; `Copiar novamente` / `Copiar e arquivar` é o fallback. O aplicativo não promete colagem Wayland nem execução de comandos.
...
- Nunca bloquear o event loop com captura de áudio, chamada de rede ou esperas ativas de thread (`QThread.wait()`).
...
- Nunca gravar chaves Gemini, áudios, base64, mensagens da sessão, alvos de terminal salvos, prompts, transcrições, mensagens de exceção brutas ou outros payloads sensíveis no banco SQLite.
...
- Nunca reutilizar janela/PID de terminal antigo sem revalidação imediata de PID e processo na allowlist.
...
`GEMINI_MODEL` é opcional e substitui o modelo padrão efetivo `gemini-3.5-flash-lite`:
...
Sem `libportaudio2`, o aplicativo ainda pode abrir, mas a gravação não conseguirá iniciar. Sem `libenchant-2-2` ou `hunspell-pt-br`, o corretor ortográfico local opera desativado de forma fail-soft com aviso explicativo nas Configurações, enquanto a gravação e a revisão com IA via Gemini continuam funcionando normalmente. Sem um backend Secret Service, a configuração feita na UI não sobrevive ao encerramento. Sem `xdotool` ou em Wayland, use `Copiar novamente` ou `Copiar e arquivar`.
...
- `tests/test_ui.py`: dois bindings ACK-gated, autorização/retomada, persistência/falha isolada, busy/close, diagnóstico permanente, engrenagem com seis grupos (`Chave API`, `Modelo Gemini`, `Atalho do mouse`, `Atalho do teclado`, `Corretor ortográfico`, `Atualizações`), botão de revisão com IA, balão flutuante de sugestões ortográficas e menu de contexto, estado de atualização Homebrew (instalação, progresso indeterminado, up-to-date, falha, diálogo de reinício, bloqueio de closeEvent durante execução e segurança contra widgets excluídos), fullscreen e grabs offscreen em ambos os tamanhos, além dos fluxos de áudio/transcrição/tokens.
...
- Wayland continua sem colagem automática do `TerminalBridge`; use os botões `Copiar novamente` ou `Copiar e arquivar` no clipboard.
...
- Mouse aceita apenas lateral/central; teclado aceita combinações seguras ou funções/mídia. Ambos servem para o fluxo de uma mão (iniciar gravação -> parar/revisar áudio -> enviar para Gemini no estado `AUDIO_READY`) e nunca suprimem o evento físico.
...
- A colagem só é permitida para processos da allowlist e exige uma janela com PID identificável (revalidando e ativando sincronamente o alvo salvo, ou colando diretamente na janela ativa detectada sem mudar foco).
- O sublinhado ortográfico local exige `libenchant-2-2` e `hunspell-pt-br` no sistema; se ausentes, o corretor opera desativado de forma fail-soft e o botão "Revisar com IA" via Gemini continua disponível.
...
Para criação de novas versões, publicação no GitHub Releases e sincronização do tap Homebrew, siga o procedimento e as regras de imutabilidade descritos em [docs/RELEASE.md](docs/RELEASE.md) e na skill [.agents/skills/falafacil-release/SKILL.md](.agents/skills/falafacil-release/SKILL.md).
