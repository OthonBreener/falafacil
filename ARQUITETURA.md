...
- Tabela `preferences(key TEXT PRIMARY KEY, value TEXT NOT NULL)`: armazena `last_microphone_identity`, `recording_mouse_button`, `recording_keyboard_shortcut` e `gemini_model` (restrito aos três IDs de `MODEL_CHOICES`: `gemini-2.5-flash-lite`, `gemini-3.5-flash-lite` ou `gemini-3.7-flash`); o schema permanece v1.
...
- `falafacil.__version__` é a fonte única de versão do pacote Python (`0.2.0`), consumida dinamicamente pelo `pyproject.toml` via `[tool.setuptools.dynamic]`, pelo `QApplication` em runtime e pelo modo `--update-probe` do bundle.
