...
- **Histórico local de consumo de tokens**: painel lateral de debug com bloco dedicado para métricas da chamada atual e totais acumulados de tokens Gemini persistidos em SQLite (armazena estritamente a allowlist de metadados permitidos: identidade do último microfone efetivamente utilizado `last_microphone_identity`, timestamp de registro `recorded_at`, modelo `model`, desfecho `outcome` como `success` ou `error` e as seis contagens independentes de tokens — entrada, saída, pensamento, cache, ferramentas e total —, preservando valores nulos ou indisponíveis quando não fornecidos pela API; a retenção é local e cumulativa, sem rotinas de limpeza ou expiração automática, sem cálculo de preço monetário nem retenção de chaves de API, arquivos de áudio PCM/WAV, payloads/previews em base64, texto do prompt, transcrições ou exceções brutas).
